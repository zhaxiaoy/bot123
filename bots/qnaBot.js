...
// Licensed under the MIT License.fffff

//const { ActivityHandler } = require('botbuilder');
const { ActivityHandler, MessageFactory,CardFactory } = require('botbuilder');
const { ActionTypes } = require('botframework-schema');
const { inserted11 } = require('./dbDao/insertDao');
/**
...
class QnABot extends ActivityHandler {
    ...
    constructor(conversationState, userState, dialog) {
        ...
        this.onMessage(async (context, next) => {
            ...
            // ------------------デバッグ用の選んだ文字を表示する-----------------------
            //const replyText = `Echo onMessage : ${ context.activity.text }`;
            //await context.sendActivity(MessageFactory.text(replyText, replyText));
            //------------------------------------------------------------------------

            // Create an array with the valid color options.
            const validColors = ['年齢', '性別'];

            // If the `text` is in the Array, a valid color was selected and send agreement.
            if (validColors.includes(context.activity.text)) {
                // Run the Dialog with the new message Activity.
                await this.dialog.run(context, this.dialogState);
            } else {
                await context.sendActivity('ご入力された 「' + context.activity.text + '」 がナレッジベースに登録されていません。');
            }
    ...
    async sendWelcomeMessage(turnContext) {
        ...
        for (const idx in activity.membersAdded) {
            if (activity.membersAdded[idx].id !== activity.recipient.id) {
                const welcomeMessage = `【ACN】チャットボットのテスト画面へようこそ！`;
     ...
     */
     async sendSuggestedActions(turnContext) {
        ...
        //         title: '年齢',
        //         value: '年齢',
        //         image: 'https://via.placeholder.com/20/FFFF00?text=1',
        //         imageAltText: '1'
        //     },
        //     {
        //         type: ActionTypes.PostBack,
        //         title: '性別',
        //         value: '性別',
        //         image: 'https://via.placeholder.com/20/FFFF00?text=2',
        //         imageAltText: '2'
        //     }
        // ];


    //     const old = CardFactory.heroCard(
    //         'White T-Shirt',
    ...
    //    );
        ...
        const sex = CardFactory.heroCard(
            '質問2：',
            [],
            ['性別']
        );

        var replyOld = MessageFactory.attachment(old, '質問を選んでください。');
        var replySex = MessageFactory.attachment(sex);

        await turnContext.sendActivity(replyOld);
        await turnContext.sendActivity(replySex);       
    }
